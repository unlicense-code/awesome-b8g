...
void NAMESPACE::fs::Unlink(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  String::Utf8Value fname(isolate, args[0]);
  args.GetReturnValue().Set(Integer::New(isolate, unlink(*fname)));
}

void NAMESPACE::fs::Realpath(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  String::Utf8Value fname(isolate, args[0]);
...
void NAMESPACE::fs::Symlink(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
...
void NAMESPACE::fs::Mknod(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  String::Utf8Value target(isolate, args[0]);
...
void NAMESPACE::fs::Mkfifo(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  String::Utf8Value target(isolate, args[0]);
...
void NAMESPACE::fs::Mount(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
...
void NAMESPACE::fs::Umount(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  String::Utf8Value target(isolate, args[0]);
  int flags = 0;
...
void NAMESPACE::fs::Open(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  String::Utf8Value fname(isolate, args[0]);
...
void NAMESPACE::fs::Chmod(const FunctionCallbackInfo<Value> &args) {
...
void NAMESPACE::fs::Chown(const FunctionCallbackInfo<Value> &args) {
  int fd = Local<Integer>::Cast(args[0])->Value();
...
void NAMESPACE::fs::Ioctl(const FunctionCallbackInfo<Value> &args) {
  int fd = Local<Integer>::Cast(args[0])->Value();
...
void NAMESPACE::fs::Ftruncate(const FunctionCallbackInfo<Value> &args) {
  int fd = Local<Integer>::Cast(args[0])->Value();
...
void NAMESPACE::fs::Fstat(const FunctionCallbackInfo<Value> &args) {
  int fd = Local<Integer>::Cast(args[0])->Value();
...
void NAMESPACE::fs::Chdir(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  String::Utf8Value path(isolate, args[0]);
  args.GetReturnValue().Set(Integer::New(isolate, chdir(*path)));
}

void NAMESPACE::fs::Rmdir(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  String::Utf8Value path(isolate, args[0]);
  args.GetReturnValue().Set(Integer::New(isolate, rmdir(*path)));
}

void NAMESPACE::fs::Rename(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  String::Utf8Value source(isolate, args[0]);
...
void NAMESPACE::fs::Mkdir(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  String::Utf8Value path(isolate, args[0]);
...
void NAMESPACE::fs::Lseek(const FunctionCallbackInfo<Value> &args) {
  int fd = Local<Integer>::Cast(args[0])->Value();
...
void NAMESPACE::fs::Readdir(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  String::Utf8Value path(isolate, args[0]);
...
void NAMESPACE::fs::FSync(const FunctionCallbackInfo<Value> &args) {
  args.GetReturnValue().Set(Integer::New(args.GetIsolate(), fsync(Local<Integer>::Cast(args[0])->Value())));
}

void NAMESPACE::fs::FDataSync(const FunctionCallbackInfo<Value> &args) {
  args.GetReturnValue().Set(Integer::New(args.GetIsolate(), fdatasync(Local<Integer>::Cast(args[0])->Value())));
}

void NAMESPACE::fs::Close(const FunctionCallbackInfo<Value> &args) {
  args.GetReturnValue().Set(Integer::New(args.GetIsolate(), 
    close(Local<Integer>::Cast(args[0])->Value())));
}

void NAMESPACE::fs::Write(const FunctionCallbackInfo<Value> &args) {
  int fd = Local<Integer>::Cast(args[0])->Value();
...
void NAMESPACE::fs::Read(const FunctionCallbackInfo<Value> &args) {
  int fd = Local<Integer>::Cast(args[0])->Value();
  Local<ArrayBuffer> buf = args[1].As<ArrayBuffer>();
...
void NAMESPACE::fs::Init(Isolate* isolate, Local<ObjectTemplate> target) {
  Local<ObjectTemplate> fs = ObjectTemplate::New(isolate);
  SET_METHOD(isolate, fs, "open", NAMESPACE::fs::Open);
  SET_METHOD(isolate, fs, "unlink", NAMESPACE::fs::Unlink);
  SET_METHOD(isolate, fs, "symlink", NAMESPACE::fs::Symlink);
  SET_METHOD(isolate, fs, "ioctl", NAMESPACE::fs::Ioctl);
  SET_METHOD(isolate, fs, "rmdir", NAMESPACE::fs::Rmdir);
  SET_METHOD(isolate, fs, "ftruncate", NAMESPACE::fs::Ftruncate);
  SET_METHOD(isolate, fs, "rename", NAMESPACE::fs::Rename);
  SET_METHOD(isolate, fs, "mkdir", NAMESPACE::fs::Mkdir);
  SET_METHOD(isolate, fs, "fstat", NAMESPACE::fs::Fstat);
  SET_METHOD(isolate, fs, "lseek", NAMESPACE::fs::Lseek);
  SET_METHOD(isolate, fs, "readdir", NAMESPACE::fs::Readdir);
  SET_METHOD(isolate, fs, "chdir", NAMESPACE::fs::Chdir);
  SET_METHOD(isolate, fs, "mount", NAMESPACE::fs::Mount);
  SET_METHOD(isolate, fs, "umount", NAMESPACE::fs::Umount);
  SET_METHOD(isolate, fs, "mknod", NAMESPACE::fs::Mknod);
  SET_METHOD(isolate, fs, "mkfifo", NAMESPACE::fs::Mkfifo);
  SET_METHOD(isolate, fs, "realpath", NAMESPACE::fs::Realpath);

  SET_METHOD(isolate, fs, "fdatasync", NAMESPACE::fs::FDataSync);
  SET_METHOD(isolate, fs, "fsync", NAMESPACE::fs::FSync);

  SET_METHOD(isolate, fs, "chmod", NAMESPACE::fs::Chmod);
  SET_METHOD(isolate, fs, "chown", NAMESPACE::fs::Chown);
  // todo: move fcntl here

  SET_METHOD(isolate, fs, "read", NAMESPACE::fs::Read);
  SET_METHOD(isolate, fs, "write", NAMESPACE::fs::Write);
  SET_METHOD(isolate, fs, "close", NAMESPACE::fs::Close);
